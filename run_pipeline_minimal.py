    ...
    if task_list == []:
        ...
        for task in task_list:
            with open(os.path.join(path, task), "r") as f:
                ...
                task_dict[task_name] = (
                    curr_task_dict["prompt_instruction"],
                    curr_task_dict["prompt_examples"],
                )
    else:
        for task in task_list:
            task = task + ".json"
...
if __name__ == "__main__":
    ...
    task_dict = get_tasks(path="task_jsons", task_list=task_list)
    ...
    for task_name, (task_description, task_examples) in task_dict.items():
        ...
        dataset = d.retrieve_dataset_dict_minimal(
            prompt_spec,
            True,
            10,
            num_votes=5,
            num_datasets_to_choose=1,
