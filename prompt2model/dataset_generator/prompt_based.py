...
class OpenAIDatasetGenerator(DatasetGenerator):
    ...
    def construct_input_output_map(
        ...
        for example in generated_examples:
            ...
            explain_str=example.explain_col
            output_str = example.output_col

            # Increment the count of the output for the specific input.
            input_output_map[input_str+"\nEXPLANATION"+explain_str][output_str] += 1
    ...
    def apply_multi_vote_to_construct_generated_dataset(
        ...
        for input_str, output_counter in input_output_map.items():
            ...
            input_break,explain_break=input_str.split("\nEXPLANATION")
            filtered_explain.append(explain_break)
            filtered_inputs.append(input_break)
        ...
        generated_dataset = Dataset.from_dict(
            {"input_col": filtered_inputs, "explain_col": filtered_explain,"output_col": filtered_outputs}
        )
        ...
        all_generated_examples_dataset = Dataset.from_dict(
            {
                ...
                "explain_col": [example.explain_col for example in generated_examples],
    ...
    def generate_dataset_split(
        ...
        if examples_cache_path.exists():
            ...
            generated_examples = [
                Example(input_col=ex["input_col"], explain_col=ex["explain_col"],output_col=ex["output_col"])
                for ex in all_generated_examples_dataset
            ]
            print(len(generated_examples))
        else:
                    ...
                    prompts = [
                        self.construct_prompt(
                            ...
                            if not self.filter_duplicated_examples
                                ...
                                Example(each["input_col"], each["explain_col"], each["output_col"])
