...
class DatasetGenerator(ABC):
    """
    An abstract class for generating datasets from a prompt specification.
    """
...
class BaseGenerator(DatasetGenerator):
    ...
    def generate_datasets(
        ...
        Returns:
            datasets.DatasetDict: Includes train, validation, and test splits.
        """
        train_examples = self.generate_examples(prompt_spec, num_train_examples, split=DatasetSplit.TRAIN)
        val_examples = self.generate_examples(prompt_spec, num_val_examples, split=DatasetSplit.VAL)
        test_examples = self.generate_examples(prompt_spec, num_test_examples, split=DatasetSplit.TEST)
